...
exports.getExpiring = async (req, res, next) => {
    try {
        ...
        const lots = await Lot.find({
            actif: true,
            quantite: { $gt: 0 },
            dateExpiration: { $lte: limit },
        }).populate('medicineId', 'nom forme dosage').sort({ dateExpiration: 1 });

        const groups = {
            expires: lots.filter(l => l.dateExpiration <= now),
            dans30j: lots.filter(l => l.dateExpiration > now && l.dateExpiration <= new Date(now.setDate(now.getDate() + 30))),
            dans60j: [],
            dans90j: [],
        };
...
exports.getAlerts = async (req, res, next) => {
    try {
        ...
        res.json({ success: true, data: alerts, unreadCount, pagination: { page: parseInt(page), limit: parseInt(limit), total } });
    } catch (err) { next(err); }
};
